...
This page provides information about monitoring user jobs. Slurm provides
different commands to gather information about current, future and past jobs. 

- `squeue` - provides information on current and future jobs
- `scontrol` - provides detailed information on currently running jobs
- `sacct` - provides detailed information on past jobs


Note that the output format of most Slurm commands is highly configurable to your needs. Look for the _--format_ or _--Format_ options in the `man` pages of the command.

## squeue

!!! tip note "Monitoring your jobs"
    Use the `squeue --me` command to get a overview of all your active (running and pending) jobs in the cluster.
...
```Bash
--me                                Display all your currently pending or running jobs
...
List all your currently running jobs:

```Bash
squeue --me --states=R
```

List all your currently running jobs in the `gpu` partition:
